...
function onlyEvenValues(arr) {
  let evenVals = arr.filter(function (num) {
    return num % 2 == 0;
  });
  return evenVals;
}
...
function filterByValue(arr, key) {
  ...
  arr.filter(function (val) {
    if (key in val) containsKey.push(val);
  });
  return containsKey;
}
...
function find(arr, searchValue) {
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] === searchValue) return arr[i];
  }
  return undefined;
}
...
function findInObj(arr, key, searchValue) {
  for (let i = 0; i < arr.length; i++) {
    if (arr[i][key] === searchValue) return arr[i];
  }
  return undefined;
}
